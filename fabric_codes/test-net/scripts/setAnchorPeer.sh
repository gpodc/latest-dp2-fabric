...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer.iot-fabric.com:7050 --ordererTLSHostnameOverride orderer.iot-fabric.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.org1.iot-fabric.com"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.org2.iot-fabric.com"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.org3.iot-fabric.com"
    PORT=11051
  else
...
updateAnchorPeer() {
  peer channel update -o orderer.iot-fabric.com:7050 --ordererTLSHostnameOverride orderer.iot-fabric.com -c $CHANNEL_NAME -f ${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
